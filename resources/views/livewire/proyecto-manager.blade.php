    ...
    @elseif($viewMode === 'form')
        ...
        <fieldset style="border: 2px solid #8b0000; border-radius: 6px; padding: 20px; background-color: #FFF;">
            ...
            <form wire:submit="save">
                ...
                {{-- == SECCIÓN INVOLUCRADOS (solo profesor proyecto) == --}}
                @if($esProfesor && $editingId)
                <fieldset style="border: 1px solid #CCC; padding: 10px; margin-bottom: 15px; background: #fafafa;">
                    ...
                    @if(!empty($involucradosProyecto))
                        <table width="100%" border="0" cellpadding="4" cellspacing="0" style="font-size: 11px; margin-bottom: 10px;">
                            ...
                            <tbody>
                                ...
                                @endphp
                                <tr style="border-bottom: 1px solid #e0e0e0; {{ $editandoEste ? 'background:#fff5f5;' : '' }}">
                                    <td style="padding: 4px 8px;">{{ $inv['nombre'] }} {{ $inv['apellido'] }}</td>
                                    <td style="padding: 4px 8px;">{{ $inv['cedula'] }}</td>
                                    <td style="padding: 4px 8px;">
                                        @if(!empty($inv['roles']))
                                    ...
                                    <td style="padding: 4px 8px;">
                                        ...
                                        <div style="display:flex; gap:4px; flex-wrap:wrap;">
                                            <button type="button" wire:click="agregarRolesAInvolucrado({{ $pivotId }}, {{ $inv['id'] }})"
                                ...
                                {{-- Panel de edición de roles para este involucrado --}}
                                @if($editandoEste)
                                    ...
                                    <td colspan="4" style="padding: 8px 12px;">
                                        <div style="border: 1px solid #8b0000; border-radius: 6px; padding: 12px; background: #fff;">
                                            <div style="font-weight: bold; font-size: 12px; color: #8b0000; margin-bottom: 8px;">
                                                Agregar más roles a: <span style="color:#333;">{{ $inv['nombre'] }} {{ $inv['apellido'] }}</span>
                                            </div>
                                            ...
                                            @if(!empty($rolesSeleccionados))
                                                <div style="margin-bottom: 8px;">
                                                    ...
                                                    <div style="margin-top: 4px;">
                                                        @foreach($rolesSeleccionados as $rId => $rNombre)
                                                            <span style="display:inline-block; background:#8b0000; color:#fff; padding:2px 10px; border-radius:10px; font-size:10px; margin:2px;">
                                                                {{ $rNombre }}
                                                                <button type="button" wire:click="quitarRolSeleccionado({{ $rId }})" style="background:none; border:none; color:#fff; cursor:pointer; margin-left:4px; font-size:12px;">&times;</button>
                                                            </span>
                                                        @endforeach
                                                    </div>
                                                </div>
                    ...
                    {{-- Formulario de selección/búsqueda de involucrado --}}
                    @if($involucradoPendienteId === null && !$mostrarFormNuevoInvolucrado)
                        <div style="margin-bottom: 8px;">
                    ...
                    @if($mostrarFormNuevoInvolucrado)
                        <div style="border: 1px solid #198754; border-radius: 6px; padding: 12px; background: #f0fff0; margin-bottom: 8px;">
                            ...
                            @error('nuevoInvolucradoNombre') <div style="color:#dc3545;font-size:10px;">⚠ {{ $message }}</div> @enderror
                            @error('nuevoInvolucradoApellido') <div style="color:#dc3545;font-size:10px;">⚠ {{ $message }}</div> @enderror
                            @error('nuevoInvolucradoCedula') <div style="color:#dc3545;font-size:10px;">⚠ {{ $message }}</div> @enderror

                            {{-- Roles para el nuevo involucrado --}}
                    ...
                    {{-- Selección de roles para el involucrado pendiente --}}
                    @if($involucradoPendienteId !== null)
                        <div style="border: 1px solid #8b0000; border-radius: 6px; padding: 12px; background: #fff5f5; margin-top: 8px;">
                            <div style="font-weight: bold; font-size: 12px; color: #8b0000; margin-bottom: 8px;">
                                Asignar roles a: <span style="color:#333;">{{ $involucradoPendienteNombre }}</span>
                            </div>
