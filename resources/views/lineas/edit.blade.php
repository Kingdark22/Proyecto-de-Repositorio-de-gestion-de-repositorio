...
@section('header', 'Editar Línea de Investigación')
...
@section('content')
    <fieldset style="border: 2px solid #8b0000; border-radius: 6px; padding: 20px; background-color: #FFF;">
        ...
        <form method="POST" action="{{ route('lineas-investigacion.update', $item->id) }}" style="margin: 0;" onsubmit="return validarFormulario(this)">
            @csrf
            @method('PUT')
            <table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin-top: 15px;">
                <tr>
                    ...
                    <td width="70%">
                                <input name="nombre_investigacion" type="text" value="{{ old('nombre_investigacion', $item->nombre_investigacion) }}" maxlength="100" style="width: 90%;" required oninput="validarNombre(this)" data-check-url="/lineas-investigacion/check-nombre">
                ...
                <tr>
                    ...
                    <td width="70%">
                        <input name="area_de_investigacion" type="text" value="{{ old('area_de_investigacion', $item->area_de_investigacion) }}" maxlength="100" style="width: 90%;" required>
                        <span class="obligatorio">*</span>
                    ...
                    <td width="70%">
                        <select name="programa_id" style="width: 90%; padding: 2px;" required>
                            ...
                            @foreach ($programas as $p)
                                <option value="{{ $p->id }}" {{ old('programa_id', $item->programa_id) == $p->id ? 'selected' : '' }}>{{ $p->siglas }} - {{ $p->nombre }}</option>
                            @endforeach
                        </select>
                        <span class="obligatorio">*</span>
                        @error('programa_id')
                ...
                <tr>
                    ...
                    <td width="70%">
                        <textarea name="descripcion" rows="3" maxlength="500" style="width: 90%;" required>{{ old('descripcion', $item->descripcion) }}</textarea>
                        <span class="obligatorio">*</span>
                        @error('descripcion')
                            <br><span class="validation-error">{{ $message }}</span>
                        @enderror
