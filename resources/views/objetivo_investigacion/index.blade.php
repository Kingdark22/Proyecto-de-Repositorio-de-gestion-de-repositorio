...
@section('content')
    ...
    <div style="margin-bottom: 15px; display: flex; align-items: center;">
        <form method="GET" action="{{ route('objetivos-investigacion') }}" style="display: inline-flex; align-items: center;">
            <b>Buscar Objetivo:</b>
            <input name="search" type="text" value="{{ $search }}"
                style="width: 400px; padding: 4px 6px; border-radius: 4px; border: 1px solid #999; margin-left: 5px;"
                placeholder="Nombre del objetivo...">
            <button type="submit" class="cm-btn cm-btn-primary"
                style="font-size: 14px; padding: 6px 16px; margin-left: 8px;">Buscar</button>
        </form>
                    ...
                    <tr
                        ...
                        <td align="center">
                            <div style="display: inline-flex; align-items: center; gap: 4px;">
                                ...
                                <button type="button"
                                    onclick="if(confirm('¿Estás seguro de cambiar el estado de este objetivo de investigación?')){window.location='{{ route('objetivos-investigacion.toggle', $item->id) }}'}"
                                ...
                                <form action="{{ route('objetivos-investigacion.destroy', $item->id) }}" method="POST"
                                    ...
                                    onsubmit="return confirm('¿Estás seguro de eliminar PERMANENTEMENTE este objetivo de investigación?');">
                                    @csrf
                                    @method('DELETE')
