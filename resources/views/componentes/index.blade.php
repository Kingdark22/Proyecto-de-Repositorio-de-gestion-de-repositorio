...
@section('content')
    ...
    <div style="margin-bottom: 15px; display: flex; align-items: center; gap: 12px;">
        <form method="GET" action="{{ route('componentes.index') }}" style="display: flex; align-items: center; gap: 8px; margin: 0;">
            <b>Buscar:</b>
            <input name="search" type="text" value="{{ $search }}" style="width: 350px; padding: 4px 6px; border-radius: 4px; border: 1px solid #999;" placeholder="Componente...">
                    ...
                    <tr style="background-color: {{ $loop->iteration % 2 == 0 ? '#E0E0E0' : '#FFFFFF' }}; {{ !$item->estado_logico ? 'color: #888;' : 'color: #000;' }}" valign="top">
                        ...
                        <td align="center">
                            <div class="cm-btn-group" style="display: inline-flex; flex-wrap: wrap; justify-content: center;">
                                ...
                                <button type="button" onclick="if(confirm('¿Cambiar estado de este componente?'))window.location='{{ route('componentes.toggle', $item->id) }}'" title="{{ $item->estado_logico ? 'Suspender' : 'Activar' }}" class="cm-btn cm-btn-warning cm-btn-sm">{{ $item->estado_logico ? 'Suspender' : 'Activar' }}</button>
                                <form method="POST" action="{{ route('componentes.destroy', $item->id) }}" style="display:inline;" onsubmit="return confirm('¿Seguro desea eliminar este componente?')">
                                    @csrf
                                    @method('DELETE')
