...
<div>
    <fieldset style="border: 2px solid #8b0000; border-radius: 6px; padding: 20px; background-color: #FFF;">
        <legend style="color: #000; font-weight: bold; font-style: italic; padding: 0 5px;">Registrar Metodolog&iacute;a</legend>

        <form method="POST" action="{{ route('metodologia-investigacion.store') }}" style="margin: 0;">
            @csrf
            <table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin-top: 15px;">
                <tr>
                    ...
                    <td width="70%">
                        <input name="nombre" type="text" value="{{ old('nombre') }}" style="width: 90%;" required>
                ...
                <tr>
                    ...
                    <td width="70%">
                        <textarea name="descripcion" rows="4" style="width: 90%;" required>{{ old('descripcion') }}</textarea>
                        <span class="obligatorio">*</span>
                        @error('descripcion')
                            <br><span class="validation-error">{{ $message }}</span>
                        @enderror
