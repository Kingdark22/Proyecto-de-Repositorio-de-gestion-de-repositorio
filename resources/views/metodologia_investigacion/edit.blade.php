...
@section('title', 'Editar Metodología de Investigación')
@section('header', 'Editar Metodología de Investigación')
...
<div>
    <fieldset style="border: 2px solid #8b0000; border-radius: 6px; padding: 20px; background-color: #FFF;">
        ...
        <form method="POST" action="{{ route('metodologia-investigacion.update', $item->id) }}" style="margin: 0;" onsubmit="return validarFormulario(this)">
            @csrf
            @method('PUT')
            <table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin-top: 15px;">
                <tr>
                    ...
                    <td width="70%">
                                <input name="nombre" type="text" value="{{ old('nombre', $item->nombre) }}" maxlength="100" style="width: 90%;" required oninput="validarNombre(this)" data-check-url="/metodologia-investigacion/check-nombre">
                ...
                <tr>
                    ...
                    <td width="70%">
                        <textarea name="descripcion" rows="4" maxlength="500" style="width: 90%;" required>{{ old('descripcion', $item->descripcion) }}</textarea>
                        <span class="obligatorio">*</span>
                        @error('descripcion')
                            <br><span class="validation-error">{{ $message }}</span>
                        @enderror
