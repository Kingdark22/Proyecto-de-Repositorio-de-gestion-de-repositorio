        ...
        <tr style="{{ $item->estado_logico ? '' : 'opacity:.5;' }}">
            ...
            <td style="text-align:center;">
                <div class="btn-actions" style="justify-content:center;">
                    <button type="button" class="btn-edit" onclick="abrirEditar('metodologias','{{ $item->id }}','{{ addslashes($item->nombre) }}')">Editar</button>
                    @if($esAdminCoord)
                    <button type="button" class="btn-delete" onclick="eliminarRegistro('metodologias','{{ $item->id }}','{{ addslashes($item->nombre) }}')">Eliminar</button>
                    @endif
                </div>
            </td>
        </tr>
